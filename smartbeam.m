    ...
    if(~track_flag)
        ...
        if((i-s_track_time)*T1 > allow_T)%确保跟踪时间不超过空白容忍时间
            ...
            if(big_beam_track_flag)
                ...
                for k = 1: object_num
                    [scan_window_l ,scan_window_w] = getScanWindow(Objects(k,4), Objects(k,5),max_big_pos_l,max_big_pos_w,map_w,big_beam,Objects(k,3),T1,0.5*T_b);
                    ...
                    if(small_beam_track_flag)
                        ...
                        if ~isempty(Small_Objects{small_track_object})
                            if(small_track_ing_flag)
                                fprintf('获取小波束跟踪扫描窗\n');
                                [scan_window_l ,scan_window_w] = getScanWindow(Small_Objects{small_track_object}(4), Small_Objects{small_track_object}(5),max_small_pos_l,max_small_pos_w,map_w,small_beam,Small_Objects{small_track_object}(3),T1,0);
